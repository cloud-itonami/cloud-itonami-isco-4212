(ns gaming.actor
  "GamingOperationsActor — the ISCO-08 4212 community gaming actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one gaming operation request
  ...
  The unconditional invariant: the GamingOperationsAdvisor can never
  directly commit a record the GamingOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [gaming.advisor :as advisor]
            [gaming.governor :as governor]
            [gaming.store :as store]))

(defn build-graph
  "Build a compiled GamingOperationsActor graph. `store` implements
  `gaming.store/Store`. `advisor` implements
  `gaming.advisor/Advisor` (defaults to `mock-advisor`).
